...
/* Рисует дерево "на боку": корень слева, ветви растут вправо. Правое
 * поддерево печатается сверху, левое — снизу; читается как повёрнутое
 * на 90° обычное дерево. prefix — накопленные вертикальные линии
 * предков, is_left — является ли узел левым потомком своего родителя. */
...
static int ask_index(const PhoneBook *pb, size_t *index)
{
    ...
    printf("Введите номер контакта (1-%zu): ", pb_count(pb));
    read_line(buf, sizeof(buf));
    n = strtol(buf, NULL, 10);
    if (n < 1 || (size_t)n > pb_count(pb)) {
    ...
    for (;;) {
        ...
        case 'S':
            if (pb_update(pb, index, &c) == PB_OK) {
                printf("Контакт обновлён (номер мог измениться "
                       "из-за сортировки).\n");
                autosave(pb);
            } else {
...
static void action_find(const PhoneBook *pb)
{
    ...
    found = malloc(pb_count(pb) * sizeof(size_t));
    if (found == NULL) {
        printf("Ошибка: не удалось выделить память.\n");
        return;
    }

    n = pb_find(pb, query, found, pb_count(pb));
    if (n == 0) {
        printf("Ничего не найдено.\n");
    } else {
        for (size_t i = 0; i < n; i++)
            print_contact(pb_get(pb, found[i]), found[i] + 1);
...
void ui_run(PhoneBook *pb)
{
    ...
    for (;;) {
        printf("\n===== ТЕЛЕФОННАЯ КНИГА =====\n"
               "1. Показать все контакты (а -> я)\n"
               "2. Показать все контакты (я -> а)\n"
               "3. Добавить контакт\n"
        ...
        switch (choice[0]) {
        case '1': action_list(pb, 0);  break;
        case '2': action_list(pb, 1);  break;
        case '3': action_add(pb);      break;
