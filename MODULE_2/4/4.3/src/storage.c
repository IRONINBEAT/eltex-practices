...
static void write_contact(FILE *f, const Contact *c)
{
    fprintf(f, "[contact]\n");
...
PbResult pb_save(const PhoneBook *pb, const char *path)
{
    ...
    n = pb_count(pb);
    snap = n > 0 ? malloc(n * sizeof(*snap)) : NULL;
    if (n > 0 && snap == NULL)
        return PB_NO_MEMORY;
    ...
    /* контакты пишутся в алфавитном порядке (in-order обход дерева) */
    n = pb_snapshot(pb, snap, n);
    for (size_t i = 0; i < n; i++)
        write_contact(f, snap[i]);
...
PbResult pb_load(PhoneBook *pb, const char *path)
{
    ...
    while (fgets(line, sizeof(line), f) != NULL) {
        ...
        if (strcmp(line, "[contact]") == 0) {
            if (in_contact)
                pb_add(pb, &c); /* контакт без фамилии/имени отсеет pb_add */
            contact_init(&c);
            in_contact = 1;
            continue;
        }
        if (!in_contact)
            continue; /* строки до первого [contact] */
    ...
    fclose(f);
    /* Файл мог быть отсортирован — тогда BST-вставки дали бы «палку».
     * Балансируем один раз после загрузки. */
    pb_balance(pb);
    return PB_OK;
}
